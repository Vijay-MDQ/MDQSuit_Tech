...
import { get_all_accommodation_expense, get_all_fuel_expense } from "../../API_Service/API_Links";
...
export default function AccomodationTable() {
  ...
  useEffect(()=>{
     axios({
          method:'GET',
          url: get_all_accommodation_expense,
      }).then(res => {
          if (res.data.error) {
  ...
  const keys = Object.keys(data[0] || {}).filter((key) => key !== "accommodationId");
