...
import { get_all_maintenance_expense, get_all_rental_expense, get_all_utilization_expense } from "../../API_Service/API_Links";
...
export default function UtilizationTable() {
  ...
  useEffect(()=>{
     axios({
          method:'GET',
          url: get_all_utilization_expense,
      }).then(res => {
          if (res.data.error) {
  ...
  const keys = Object.keys(data[0] || {}).filter((key) => key !== "utilizationId");
